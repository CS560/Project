...
public class NHTSAListActivity extends ListActivity {
		...
		rlv.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				String link = ((TextView) view.findViewById(R.id.link))
						.getText().toString();
				String keywords = ((TextView) view
						.findViewById(R.id.manufacturer)).getText().toString();
				// Starting new intent
				Intent in = new Intent(getApplicationContext(),
						SingleNHTSAItemActivity.class);
	...
	private class GetRecalls extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... arg0) {
			...
			System.out.println("1. " + org + "2. " + cg + "3. " + mf + "4. "
					+ srh);
			if (mf.equals("") && srh.equals("")) {
				URL = "http://134.193.136.127:8983/solr/group6_shard1_replica1/select?q=category%3A"
						+ org
						+ "+AND+resourcename%3A"
						+ cg
						+ "%0A&rows=200&sort=last_modified+desc&wt=json";
			} else if (srh.equals("")) {
				URL = "http://134.193.136.127:8983/solr/group6_shard1_replica1/select?q=category%3A"
						+ org
						+ "+AND+resourcename%3A"
				...
				URL = "http://134.193.136.127:8983/solr/group6_shard1_replica1/select?q=category%3A"
						...
						+ cg
						+ "+AND+subject%3A"
						+ srh + "%0A&rows=200&sort=last_modified+desc&wt=json";
			} else if (cg.equals("")) {
				URL = "http://134.193.136.127:8983/solr/group6_shard1_replica1/select?q=category%3A"
						+ org
						+ "+AND+description%3A"
		...
		protected void onPostExecute(Void result) {
			...
			if (js != null) {
				try {
					...
					System.out.println(numr);
					if (numr.equals("0")) {
						alt_bld = new AlertDialog.Builder(
								NHTSAListActivity.this);
						...
						ListAdapter adapter = new SimpleAdapter(
								...
								R.layout.recent_recall_view, new String[] {
										Title, Date, Description, Link,
										Keywords }, new int[] { R.id.title,
										R.id.pubDate, R.id.description,
										R.id.link, R.id.manufacturer });
